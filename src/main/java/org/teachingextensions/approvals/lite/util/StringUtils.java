...
public class StringUtils {
  ...
  public static String stripNonNumeric(String number, boolean allowDecimal,
  ...
  /**
   * A convenience function to turn a vector of String objects into an Array of
   * the String objects.
   *
   * @param vectorOf
    ...
    while (iterator.hasNext()) {
      ...
      if (rowObject != null) {
        ...
        throw new Error("toArray[" + i
            + "] is not an instance of String but a "
  ...
  public static <T> String toString(String name, T[] array) {
    ...
    if (array == null || array.length == 0) {
      ...
      for (int i = 0; i < array.length; i++) {
        buffer.append(name).append("[").append(padNumber(i, maxPadding))
  ...
  public static <T> String toString(String name, Iterable<T> array) {
    ...
    if (array != null) {
      int count = 0;
      for (T t : array) {
        buffer.append(name).append("[").append(count).append("] = ")
            .append(arrayStringHelper(t)).append("\r\n");
        count++;
      }
    }
    if (buffer.length() == 0) {
