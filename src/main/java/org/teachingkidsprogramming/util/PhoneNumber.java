...
public class PhoneNumber {
    ...
    private static String getValueAsNorthAmerican(int countryCode, String stripped) {
        ...
        StringBuilder number = new StringBuilder(stripped.substring(2));
        number.insert(6, "-");
        number.insert(3, ")");
        number.insert(0, "(");
    ...
    private static String getValueAsInternational(int countryCode, String stripped) {
        ...
        StringBuilder number = new StringBuilder(stripped);
        int internationalLen = COUNTRY_CODES[countryCode].length() + 1;
        int bodyLength = extractPhoneBody(countryCode, stripped).length();
        if (bodyLength > 6) {
            number.insert(internationalLen + 6, ".");
        }
        if (bodyLength > 3) {
            number.insert(internationalLen + 3, ".");
        }
        number.insert(internationalLen, ".");
        return number.toString();
    }
