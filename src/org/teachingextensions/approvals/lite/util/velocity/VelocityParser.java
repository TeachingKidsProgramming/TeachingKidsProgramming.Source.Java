...
public class VelocityParser {
  ...
  public static String parseString(String template, ContextAware process) {
    ...
    props.put("runtime.introspector.uberspect",
        TestableUberspect.class.getName());
    props.put("velocimacro.permissions.allow.inline.local.scope", "" + true);
    return parse(template, props, new ContextAware[] { process,
        Default.INSTANCE });
  }
  ...
  public static synchronized VelocityEngine initializeEngine(Properties props)
    ...
    if (currentEngine == null
        || isDifferentForProperties(props, currentEngine, new String[] {
            "resource.loader", "file.resource.loader.path" })) {
  ...
  private static boolean isDifferentForProperties(Properties props,
      VelocityEngine engine, String[] keys) {
    for (String key : keys) {
  ...
  public static String parseFromClassPath(Class<?> clazz, String string,
